...
public class LeProxy {
    ...
    private String dex16Command = "";
    private BleService mBleService;
    private boolean mEncrypt = false;
    private HashSet<HashMap<String, String>> result = new HashSet<>();
    ...
    public boolean send(String address, UUID serUuid, UUID charUuid, byte[] data) {
        if (mBleService != null) {
            this.dex16Command = DataUtil.byteArrayToHex(data);
     ...
     * 向指定通道发数据
     *
     * @param dex16    16位的请求命令
     * @param address  设备地址
     * @param serUuid  服务uuid
     * @param charUuid 特征uuid
     * @param data     发送的数据
     */
    public boolean send(String dex16, String address, UUID serUuid, UUID charUuid, byte[] data) {
        if (mBleService != null) {
            this.dex16Command = dex16;
            BluetoothGatt gatt = mBleService.getBluetoothGatt(address);
            BluetoothGattCharacteristic c = GattUtil.getGattCharacteristic(gatt, serUuid, charUuid);
            return mBleService.send(gatt, c, data, mEncrypt);
        }
...
//            Log.d(TAG, "receiver: onCharacteristicChanged:" + System.currentTimeMillis());

            HashMap<String, String> map = new HashMap<>();
            map.put("command", dex16Command);
            map.put("result", DataUtil.byteArrayToHex(data));
            result.add(map);

            Log.d(TAG, "onCharacteristicChanged: ----command:"+dex16Command+"--result:"+DataUtil.byteArrayToHex(data));
        ...
        public void onNotifyStateRead(UUID uuid, UUID uuid1, boolean b) {
            Log.i(TAG, "onNotifyStateRead   uuid() - " + uuid + ", "
                    + "\n uuid1=" + uuid1
                    + " [" + b + ']');
        ...
        public void onCharacteristicChanged(String s, byte[] bytes) {
            Log.i(TAG, "onCharacteristicChanged  s - " + s + ", "
                    + "\n bytes=" + bytes.length
            );
        }
        ...
        public void onCharacteristicRead(String s, byte[] bytes, int i) {
            Log.i(TAG, "onCharacteristicRead  s - " + s + ", "
                    + "\n bytes=" + bytes.length
            );
        }
        ...
        public void onDescriptorRead(String s, BluetoothGattDescriptor bluetoothGattDescriptor, int i) {
            Log.i(TAG, "onDescriptorRead  s - " + s + ", "
            );
        }
...
//                    Log.i(TAG, "Enable 0x1002 notification: " + success);
                    ...
                    // String service = "FFF0"; 0000fff0-0000-1000-8000-00805f9b34fb
                    // String channel = "FFF1"; 0000fff1-0000-1000-8000-00805f9b34fb
                    boolean success = enableNotification(address, UUID.fromString("0000fff0-0000-1000-8000-00805f9b34fb"), UUID.fromString("0000fff1-0000-1000-8000-00805f9b34fb"));
                    Log.i(TAG, "Enable 0x1002 notification:renogy " + success);
                    break;
                default:
                    cancelTask();
                    break;
